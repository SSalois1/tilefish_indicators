...
sfob.env <- sfobs %>% 
  ...
  select(DATE, YEAR, MONTH, YDAY,trip_id,hull_num, area, effort_dur, 
         SUM_GT_CATCH, cpue_hr, mesh_size, depth, start_lat, start_lon, 
...
Figure 2. Catch-per-unit-effort for undirected trawl trips from the Study fleet and observer program. Zeros have been added using species association methodology (via jaccard index).
...
# Not a bad idea: It's a hard thing to justify for a peer reviewed process, but as part of an exploratory analysis, I see no problems with trying it out. I do know there are lots of studies out there working on better ways to analyze causal mechanisms of these periodic recruitment ts. But if we only have 4-5 strong year classes, that's not a lot of data to work with.
```
...
# Question: How come the strong year class lines don't seem to match all the peaks in the data?
```
...
Adelle comment: The more I look these, the less I'm sure what to do with it. Can we maybe sum across all these small sizes to produce an average yearly estimate?
...
Adelle comment: As we discussed, this might be a problem or we can do it by strata
