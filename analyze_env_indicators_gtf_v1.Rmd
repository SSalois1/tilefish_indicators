...
#filter sfob.env 
shlfvol.sfob = sfob.env %>% filter(depth > 50) %>%
  group_by(year,month) %>% 
  summarise(mean_dpth = mean(depth),
            mean_bt = mean(bottomt),
            mean_cpue = mean(cpue_hr))
...
ggscatter(df.shlfvol.sfob, x = "mean_cpue", y = "mean_v", 
          ...
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Mean CPUE/hr", ylab = "Mean shelf water volume")
...
ggscatter(df.shlfvol.sfob, x = "mean_cpue", y = "mean_t", 
          ...
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Mean CPUE/hr", ylab = "Mean shelf water temperature")
...
ggscatter(df.shlfvol.sfob, x = "mean_cpue", y = "mean_s", 
          ...
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Mean CPUE/hr", ylab = "Mean shelf water salinity")
...
gsi.sfob<-mutate(gsi, pos = ifelse(GSI > 0, 'Northerly', 'Southerly'), 
...
#GSI Apr-July
filter(gsi.sfob, month %in% c(4:7)) %>%
...
#ANOVA
pos_aov_cpue <- aov(mean_cpue ~ year * pos,
              data = gsi.sfob)
