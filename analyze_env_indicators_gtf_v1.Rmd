...
df <- dplyr::full_join(recruit, sst.lag %>%
                   group_by(year) %>% 
                   filter(year %in% c(2000:2020), month %in% c(8,9)),
...
## SST no lag
ggplot2::ggplot(sst.rec,
                aes(x=recruit_est, y=weighted_mean_sst)) + 
...
print(paste0('correlation: ', round(cor(sst.rec[,2], sst.rec[,4]), 4)))
```
...
#bt_gam <- gam(recruit_est ~ s(mean_bt), data=df.join.bt, method="REML")
...
## shelf water volume
lm_shlfvol<-lm(mean_v ~ recruit_est, data=df.join.shlf)
summary(lm_shlfvol)
cor.test(df.join.shlf$recruit_est, df.join.shlf$mean_v, method=c("pearson"))

ggscatter(df.join.shlf, x = "recruit_est", y = "mean_v", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Recruitment Estimate", ylab = "Mean shelf water volume")

#shlfvol_gam <- gam(recruit_est ~ s(mean_v), data=df.join.shlf, method="REML")
...
#shlftemp_gam <- gam(recruit_est ~ s(mean_t), data=df.join.shlf, method="REML")
#draw(shlftemp_gam, residuals=TRUE)
...
shlfsal_gam <- gam(recruit_est ~ s(mean_s, k=5), data=df.join.shlf, sp=0.01)
draw(shlfsal_gam, residuals=TRUE)
```
...
lm_gsi<-lm(GSI ~ recruit_est, data=df.cor)
summary(lm_gsi)
cor.test(df.cor$recruit_est, df.cor$GSI, method=c("pearson"))

ggscatter(df.cor, x = "recruit_est", y = "GSI", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Recruitment Estimate", ylab = "Gulf Stream Position Anomaly")

gsi_gam <- gam(recruit_est ~ s(GSI, k=5), data=df.cor, sp=0.01)
