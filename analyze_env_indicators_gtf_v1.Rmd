...
# GAM
sst_gam <- gam(weighted_mean_sst ~ s(mean_cpue), data=sst.sfob, method="REML")
#gam.check(sst_gam)
#summary(sst_gam)
...
```{r}
bt_gam <- gam(mean_bt ~ s(mean_cpue), data=df.lag.sfob, method="REML")
#gam.check(bt_gam)
#summary(bt_gam)
...
```{r}
#shelf vol GAM
shlfvol_gam <- gam(mean_v ~ s(mean_cpue), data=df.shlfvol.sfob, method="REML")
...
#GSI GAM
gsi_gam <- gam(m.gsi ~ s(recruit_est), data=gsi_df, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
#GSI GAM
gsi_gam1 <- gam(mean_gsi_lag1 ~ s(recruit_est), data=gsi_df, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
## 2 year lag
lm_rec_gsi2<-lm(mean_gsi_lag2 ~ recruit_est, data=gsi_df)
summary(lm_rec_gsi2)
...
#GSI GAM
gsi_gam2 <- gam(mean_gsi_lag2 ~ s(recruit_est), data=gsi_df, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
#GSI GAM
gsi_gam3 <- gam(mean_gsi_lag3 ~ s(recruit_est), data=gsi_df, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
#GSI GAM
gsi_gam6 <- gam(mean_gsi_lag6 ~ s(recruit_est), data=gsi_df, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
**CPUE/Year lags**
```{r}
cpue_yr<-read.csv(here::here('data/catch_data/cpue_yr.csv'))

sfob_yr<- dplyr::full_join(cpue_yr, gsi.m %>%
                   group_by(year) %>% 
                   filter(month %in% c(3:8)) %>% 
                   summarise(m.gsi = mean(GSI),
                             sd.gsi = sd(GSI),
  ...
  mutate(pos = ifelse(m.gsi > 0, 'Northerly', 'Southerly'), 
         n.pos = ifelse(m.gsi > 0, 1, 0))
gsi_yr <- sfob_yr[-c(26:69),] 

## No lag
lm_cpue_gsi<-lm(m.gsi ~ mean_cpue_yr, data=gsi_yr)
summary(lm_cpue_gsi)

cor.test(gsi_yr$mean_cpue_yr, gsi_yr$m.gsi, method=c("pearson"))

ggscatter(gsi_yr, x = "mean_cpue_yr", y = "m.gsi", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson", label.x=10, label.y=-1.0),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Mean Yearly CPUE", ylab = "Mean Yearly GSI",
          title="GSI no lag")

#GSI GAM
gsi_cpue_gam <- gam(m.gsi ~ s(mean_cpue_yr), data=gsi_yr, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
## 1 year lag
lm_cpue_gsi1<-lm(mean_gsi_lag1 ~ mean_cpue_yr, data=gsi_yr)
summary(lm_cpue_gsi1)

cor.test(gsi_yr$mean_cpue_yr, gsi_yr$mean_gsi_lag1, method=c("pearson"))

ggscatter(gsi_yr, x = "mean_cpue_yr", y = "mean_gsi_lag1", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson", label.x=10, label.y=-1.0),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Mean Yearly CPUE", ylab = "Mean Yearly GSI",
...
#GSI GAM
gsi_cpue_gam1 <- gam(mean_gsi_lag1 ~ s(mean_cpue_yr), data=gsi_yr, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
## 2 year lag
lm_cpue_gsi2<-lm(mean_gsi_lag2 ~ mean_cpue_yr, data=gsi_yr)
summary(lm_cpue_gsi2)

cor.test(gsi_yr$mean_cpue_yr, gsi_yr$mean_gsi_lag2, method=c("pearson"))

ggscatter(gsi_yr, x = "mean_cpue_yr", y = "mean_gsi_lag2", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson", label.x=10, label.y=-1.0),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Mean Yearly CPUE", ylab = "Mean Yearly GSI",
...
#GSI GAM
gsi_cpue_gam2 <- gam(mean_gsi_lag2 ~ s(mean_cpue_yr), data=gsi_yr, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
## 3 year lag
lm_cpue_gsi3<-lm(mean_gsi_lag3 ~ mean_cpue_yr, data=gsi_yr)
summary(lm_cpue_gsi3)

cor.test(gsi_yr$mean_cpue_yr, gsi_yr$mean_gsi_lag3, method=c("pearson"))

ggscatter(gsi_yr, x = "mean_cpue_yr", y = "mean_gsi_lag3", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson", label.x=10, label.y=-1.0),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Mean Yearly CPUE", ylab = "Mean Yearly GSI",
...
#GSI GAM
gsi_cpue_gam3 <- gam(mean_gsi_lag3 ~ s(mean_cpue_yr), data=gsi_yr, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
#GSI GAM
gsi_gam <- gam(GSI ~ s(mean_cpue), data=gsi.sfob, method="REML")
#gam.check(gsi_gam)
#summary(gsi_gam)
...
# GAM
micro_gam <- gam(weighted_mean_micro ~ s(mean_cpue), data=micro.sfob, method="REML")
#gam.check(micro_gam)
#summary(micro_gam)
...
```{r}
chl_gam <- gam(weighted_mean_chl ~ s(mean_cpue), data=chl.sfob, method="REML")
#gam.check(chl_gam)
#summary(chl_gam)
...
fprob_gam <- gam(weighted_mean_fprob ~ s(mean_cpue), data=fprob.sfob, method="REML")
#gam.check(fprob_gam)
#summary(fprob_gam)
