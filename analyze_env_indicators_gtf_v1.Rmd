...
sfob <- sfob.env %>% filter(depth > 50, cpue_hr >0, cpue_hr < 20) %>%
  group_by(year,month) %>% 
...
df.lag.cpue = sfob.env %>% filter(year > 2006 & depth > 50 & cpue_hr > 0 & cpue_hr < 20) %>%
  group_by(year,month) %>% 
...
#filter sfob.env 
shlfvol.sfob = sfob.env %>% filter(depth > 50, cpue_hr > 0, cpue_hr<20) %>%
  group_by(year,month) %>% 
  summarise(mean_dpth = mean(depth),
            mean_bt = mean(bottomt),
            mean_cpue = mean(cpue_hr))
...
ggscatter(shlfvol_nolag, x = "mean_cpue", y = "mean_v", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
...
ggscatter(shlfvol_nolag, x = "mean_cpue", y = "mean_t", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
...
ggscatter(shlfvol_nolag, x = "mean_cpue", y = "mean_s", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
...
## 5 year lag
lm_rec_gsi5<-lm(mean_gsi_lag5 ~ recruit_est, data=gsi_df)
summary(lm_rec_gsi5)

cor.test(gsi_df$recruit_est, gsi_df$mean_gsi_lag5, method=c("pearson"))

ggscatter(gsi_df, x = "recruit_est", y = "mean_gsi_lag5", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson", label.x=10, label.y=-1.0),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Recruitment estimate", ylab = "Mean Yearly GSI",
          title = "GSI 5 year lag")
...
ggscatter(gsi_df, x = "recruit_est", y = "mean_gsi_lag7", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson", label.x=10, label.y=-1.0),
          add.params=list(color="dodgerblue", fill="lightgray"),
          xlab = "Recruitment estimate", ylab = "Mean Yearly GSI",
          title = "GSI 7 year lag")
...
gsi_nozero <- filter(gsi, mean_cpue > 0, mean_cpue<20) #removes outlier
...
```{r}
## habitat (sst/bt/sal?)
habitat <- dplyr::full_join(sst.sfob, df.lag.cpue, by = join_by(year, month, mean_cpue)) %>%
  dplyr::select(year, month, mean_cpue, weighted_mean_sst, mean_bt)


habitat.gam <- gam(mean_cpue ~ s(weighted_mean_sst, k=10) + s(mean_bt, k=10), data=habitat, method="REML")
plot(habitat.gam)

## currents (shelf vol./GSI)
currents <- dplyr::full_join(shlfvol_nolag, gsi.sfob, by = join_by(year, month, mean_cpue)) %>%
  dplyr::select(year, month, mean_cpue, mean_v, GSI)

currents1 <- read.csv(here::here('data/currents.csv')) #cleaned up

currents.gam <- gam(mean_cpue ~ s(mean_v, k=10) + s(GSI, k=10), data=currents1, method="REML")
plot(currents.gam)

## food availability
food <- dplyr::full_join(micro.sfob, chl.sfob, by = join_by(year, month, mean_cpue)) %>%
  dplyr::select(year, month, mean_cpue, weighted_mean_micro, weighted_mean_chl)
#join micro and chl

#fprob is by season, added to food csv

food1 <- read.csv(here::here('data/food.csv')) #fprob is by season, added to food csv

food.gam <- gam(mean_cpue ~ s(weighted_mean_micro, k=10) + s(weighted_mean_chl, k=10) + s(weighted_mean_fprob, k=10), data=food1, method="REML")
