...
df.lag1 = mutate(df.join.shlf, mean_v_lag1 = lag(mean_v,12)) #creating one year lag
...
## shelf water volume - one year lag
lm_lag<-lm(mean_v_lag1 ~ recruit_est, data=df.lag1)
summary(lm_lag)
cor.test(df.lag1$recruit_est, df.lag1$mean_v_lag1, method=c("pearson"))

ggscatter(df.lag1, x = "recruit_est", y = "mean_v_lag1", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
...
## GSI - one year lag
lm_lag<-lm(gsi_lag1 ~ recruit_est, data=df.gsi)
summary(lm_lag)
cor.test(df.gsi$recruit_est, df.gsi$gsi_lag1, method=c("pearson"))

ggscatter(df.gsi, x = "recruit_est", y = "gsi_lag1", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coeff.args = list(method="pearson"),
          add.params=list(color="dodgerblue", fill="lightgray"),
